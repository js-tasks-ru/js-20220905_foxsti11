export default class RangePicker {
    ...
    if (!this.selectedButtonCalendarGrid) {
      ...
      while (selectedDate <= this.range.to) {
        const selectedCell = calendar.querySelector(
          `[data-value="${selectedDate.toISOString()}"]`
        );
  ...
  initEventListners() {
    document.body.addEventListener(
      "pointerdown",
      this.onDocumentPointerdownHandler
    );
    this.subElements.input.addEventListener(
      "click",
      this.onInputPointerdownHandler
    );
    this.subElements.selector.addEventListener(
      "click",
      this.onSelectorPointerdownHandler
    );
  }
  ...
  onSelectorPointerdownHandler = (event) => {
    const { selector } = this.subElements;
    const leftArrowControl = selector.querySelector(
      ".rangepicker__selector-control-left"
    );
    const rightArrowControl = selector.querySelector(
  ...
  changeCalendars(currentDirection, calendars) {
    ...
    this.currentDateShow.setMonth(
      this.currentDateShow.getMonth() + directions[currentDirection]
    );
    ...
    if (currentDirection === "left") {
      ...
      const wrapper = document.createElement("div");
      wrapper.innerHTML = this.getTemplateSelectorCalendar(
    ...
    if (!this.selectedButtonCalendarGrid) {
      ...
      if (dateTo < dateFrom) {
        ...
        this.range.from = new Date(
          this.selectedButtonCalendarGrid.dataset.value
        );
